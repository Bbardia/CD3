...
- Local Mac users who want a private tool without cloud infrastructure.
...
4. **Useful offline.** CD3 is built on a Raspberry Pi but intended to run locally on a Mac; services remain loopback-only and require no remote service.
...
- shared semantic selection across separate 2D and orthographic 3D modes and the model navigator;
- orthographic-isometric 3D, with split mode and perspective controls deferred to later production interaction work;
...
The code is built on a Raspberry Pi but is intended to run locally on a Mac. All services bind only to the loopback interface; CD3 has no cloud runtime dependency and must not be exposed publicly. The source is hosted in the private repository at `github.com/Bbardia/CD3`. Project data will be stored as schema-versioned JSON snapshots; databases, project data, backups, secrets, and machine-specific paths are never committed. Representative-Mac performance profiling remains pending, so this milestone claims no Mac performance measurements.
...
- split-mode presentation, perspective camera controls, or production interaction polish;
...
4. Separate 2D and orthographic 3D modes render from the sample and share selection.
